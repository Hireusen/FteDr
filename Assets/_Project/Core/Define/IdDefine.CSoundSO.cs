#pragma warning disable IDE1006

public static partial class Id
{
    public const string BGM_Blue_Water = "BGM_Blue_Water";
    public const string BGM_Cleyton_RX_Underwater = "BGM_Cleyton RX Underwater";
    public const string BGM_Observing_The_Star = "BGM_Observing The Star";
    public const string BGM_Ocean_2_Loop = "BGM_Ocean 2 Loop";
    public const string BGM_PeriTune_Oceanfront_Loop = "BGM_PeriTune Oceanfront Loop";
    public const string BGM_Peritune_Portside_Cafe_Loop = "BGM_Peritune Portside Cafe Loop";
    public const string BGM_Underwater_Exploration = "BGM_Underwater Exploration";
    public const string BGM_Underwater_Theme_II = "BGM_Underwater Theme II";
    public const string BGM_サカナだった頃 = "BGM_サカナだった頃";
    public const string BGM_氷晶と海 = "BGM_氷晶と海";
    public const string BGM_海の中の旋律 = "BGM_海の中の旋律";
    public const string BGM_珊瑚礁 = "BGM_珊瑚礁";
    public const string SFX_Attack_03 = "SFX_Attack 03";
    public const string SFX_Button = "SFX_Button";
    public const string SFX_Clank1 = "SFX_Clank1";
    public const string SFX_Dive_02 = "SFX_Dive 02";
    public const string SFX_Jump_03 = "SFX_Jump 03";
    public const string SFX_robotics2 = "SFX_robotics2";
    public const string SFX_Sonar_Ping = "SFX_Sonar Ping";
    public const string SFX_spacestationhum02seamless = "SFX_spacestationhum02seamless";
    public const string SFX_Swim_Submerged_02 = "SFX_Swim Submerged 02";
    public const string SFX_Teleport_02 = "SFX_Teleport 02";
    public const string SFX_Underwater_Engine = "SFX_Underwater Engine";
    public const string SFX_v2_metal_02 = "SFX_v2_metal_02";
}
