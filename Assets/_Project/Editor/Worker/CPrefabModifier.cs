...
public class CPrefabModifier : EditorWindow
{
    ...
    private void Execute()
    {
        ...
        if (roots.Length == 0)
        {
            UDebug.Print("선택된 오브젝트가 없습니다.", LogType.Warning);
            return;
        }
        ...
        UDebug.Print($"작업 완료: 루트 {rootCount}개, 대상 {affectedCount}개에 적용했습니다.");
    }
    ...
    private bool TryResolveOperationType(out Type opType, out Type filterType)
    {
        ...
        if (_filterByComponent)
        {
            ...
            if (filterType == null)
            {
                UDebug.Print("탐색 조건 컴포넌트의 타입을 찾을 수 없습니다.", LogType.Error);
                return false;
            }
        }
            ...
            || _operation == EOperation.SetProperty)
        {
            ...
            if (opType == null)
            {
                UDebug.Print("작업 컴포넌트의 타입을 찾을 수 없습니다.", LogType.Error);
                return false;
            }
        }
    ...
    private void RefreshPropertyList()
    {
        ...
        if (opType == null)
        {
            UDebug.Print("컴포넌트 타입을 찾을 수 없습니다.", LogType.Error);
            return;
        }
        ...
        if (sample == null)
        {
            UDebug.Print("선택된 프리팹에서 해당 컴포넌트를 찾지 못했습니다.", LogType.Warning);
            return;
        }
