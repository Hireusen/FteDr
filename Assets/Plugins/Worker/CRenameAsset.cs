...
public class CRenameAsset : EditorWindow
{
    ...
    private void ExecuteRename()
    {
        ...
        {
            Debug.LogWarning("[CRenameAsset.cs] 선택한 에셋이 없습니다.");
            return;
        }
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            {
                Debug.LogError($"[CRenameAsset.cs] {oldName}의 이름을 변경하지 못했습니다.\n{errorMsg}", obj);
            }
        }

        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        Debug.Log($"[CRenameAsset.cs] 총 {successCount}개의 에셋 이름을 변경했습니다.");
    }
    #endregion
